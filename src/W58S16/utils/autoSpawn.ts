...
const autoSpawn = {
    spawn: function (Spawn: StructureSpawn) {
        ...
        for (const roleDef of roleDefinitions) {
            ...
            const maxCount = roleDef.expectedCount;
            let currentCount = _.filter(Game.creeps, (creep) => creep.memory.role === role).length;

            // 修改此处的判断条件，使得每个角色都能生成到期望数量
            while (currentCount < maxCount) {
                const newName = `${role}_|_${Game.time}`;
                const parts: BodyPartConstant[] = expandBodyParts(roleDef.bodyParts); // 使用展开后的身体部件数组
                const result = Spawn.spawnCreep(parts, newName, {
                    memory: {role: role, room: '', working: false},
                });
                // result 的值将会是一个生成 creep 的状态码
                // 可以根据 result 的值来处理生成 creep 的结果
                if (result === OK) {
                    console.log(`正在生成 ${role}，名称：${newName}`);
                } else {
                    console.log(`生成 ${role} 失败，状态码：${result}`);
