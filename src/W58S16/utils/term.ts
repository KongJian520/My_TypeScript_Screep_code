const terminalW58S16 = {
    send: function (terminal: StructureTerminal) {
        ...
        processOrder(Game.market.getAllOrders({
            type: ORDER_BUY,
            resourceType: goods
        }).sort((a, b) => b.price - a.price), 500, terminal)

        processOrder(Game.market.getAllOrders({
            type: ORDER_SELL,
            resourceType: RESOURCE_ENERGY
        }).sort((a, b) => a.price - b.price), 500, terminal)
        ...
        function processOrder(orderSell: Order[], OrderAmount: number, terminal: StructureTerminal) {
            for (let i = 0; i < 5; i++) {
                if (terminal.store[goods] >= orderSell[i].amount) {
                    ...
                    console.log("出售结果：");
                    // 计算交易所需能量
                    const orderNeededEnergy = Game.market.calcTransactionCost(orderSell[i].amount, terminal.room.name, orderSell[i].roomName as string);
                    ...
                    console.log(`${orderSell[i].type}不够，中断代码执行`);
                }
            }
        }
