const terminalW58S16 = {
    send: function () {
        ...
        const order_buy = Game.market.getAllOrders({type: ORDER_BUY, resourceType: goods});//购买的订单
        const order_sell = Game.market.getAllOrders({type: ORDER_SELL, resourceType: RESOURCE_ENERGY});//出售的订单
        order_buy.sort((a: Order, b: Order) => b.price - a.price);
        order_sell.sort((a: Order, b: Order) => a.price - b.price);
        const order_buyneededEnergy = Game.market.calcTransactionCost(order_buy[0].amount, W58S16, order_buy[0].roomName as string);
        const order_sellneededEnergy = Game.market.calcTransactionCost(order_sell[0].amount, W58S16, order_buy[0].roomName as string);
        console.log('======待处理订单：======')
